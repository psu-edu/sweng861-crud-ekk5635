...
# Name of the short-lived cookie that carries the CSRF state between the
...
def login() -> RedirectResponse:
    ...
    The cookie is HttpOnly (script cannot read it), SameSite=Lax (sent on the
    top-level redirect back from Google, but not on cross-site subrequests),
    scoped to /auth (never attached to API calls), and Secure whenever the
    redirect URI is https - http is only ever used for localhost.
    """
